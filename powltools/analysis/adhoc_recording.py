...
from ..analysis.recording import GlobalParamsTypeV0, GlobalParamsType
    ...
    *   You can manually set threshold
    ...
    def samplingrate(self) -> float:
        if self.powl_version == "0.0.0":
            global_parameters = cast(GlobalParamsTypeV0, self.global_parameters())
            return global_parameters["adc_samplingrate"]
        else:
            global_parameters = cast(GlobalParamsType, self.global_parameters())
            return global_parameters["samplingrates"]["traces"]
