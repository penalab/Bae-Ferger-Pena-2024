...
class POwlFile:
    ...
    def __repr__(self) -> str:
        return f"POwlFile({str(self.filepath)!r}, mode={self.mode!r})"
    ...
    def trials(self) -> Iterator[tuple[int, h5py.Group]]:
        ...
        """
        with self as file:
            trials_group = cast(h5py.Group, file.get(self.KEY_TRIALS))
            for trial_index in self.trial_indexes:
                yield trial_index, cast(h5py.Group, trials_group[f"{trial_index}"])

    @cached_property
    def trial_indexes(self) -> list[int]:
        with self as file:
            return sorted(
                int(k) for k in cast(h5py.Group, file[self.KEY_TRIALS]).keys()
            )
    ...
    def get_pause(self, pause_index: int) -> h5py.Group:
        ...
        with self as file:
            try:
                return cast(
                    h5py.Group,
                    cast(h5py.Group, file[self.KEY_PAUSES])[f"{pause_index}"],
                )
            except KeyError:
                raise PauseNotFound(pause_index, self)
    ...
    def channel_numbers(
        ...
        with self as file:
            group = cast(
                h5py.Group, file[f"{self.KEY_TRIALS}/{trial_index}/{signal_key}"]
            )
...
class PauseNotFound(KeyError):
    ...
    def __str__(self) -> str:
        ...
        return "PauseNotFound for unknown reason"
